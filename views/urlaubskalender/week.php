...
<table id="main_content" class="calendar-week">
    ...
    <tr>
        <td colspan="<?= $colspan_2 ?>" style="vertical-align: middle; text-align: center;">
            <div style="text-align: left; width: 25%; display: inline-block; white-space: nowrap;">
                <a href="<?= $controller->url_for('urlaubskalender/week', array('atime' => strtotime('-1 week', $atime))) ?>">
    ...
    <tr>
        <td style="text-align: center; white-space: nowrap;" <?= $colspan_1 ?>>
            <? if ($start > 0) : ?>
            <a href="<?= $controller->url_for('urlaubskalender/week', array('atime' => mktime($start - 1, 0, 0, date('n', $atime), date('j', $atime), date('Y', $atime)))) ?>">
        ...
        <td style="text-align:center; font-weight:bold;"<?= ($tab_arr[$i]['max_cols'] > 0 ? ' colspan="' . ($tab_arr[$i]['max_cols'] + 1) . '"' : '' ) ?>>
            <a class="calhead" href="<?= $controller->url_for('urlaubskalender/day', array('atime' => $calendars[$i]->getStart())) ?>">
        ...
        <td style="text-align: center; white-space: nowrap;" <?= $colspan_1 ?>>
            <? if ($start > 0) : ?>
            <a href="<?= $controller->url_for('urlaubskalender/week', array('atime' => mktime($start - 1, 0, 0, date('n', $calendars[0]->getStart()), date('j', $calendars[0]->getStart()), date('Y', $calendars[0]->getStart())))) ?>">
                <?= Icon::create('arr_1up', 'clickable', ['title' => _('Früher')])->asImg() ?>
            </a>
            <? endif ?>
        </td>
    ...
    <tr>
        ...
        ?>
        <?= $date_template_engine->render('calendar/single/_day_dayevents', array('controller' => $controller, 'em' => $tab_arr[$i], 'calendar' => $calendars[$i], 'class_cell' => $class_cell)) ?>
        <? endfor ?>
    ...
    <tr>
        ...
        <? for ($y = 0; $y < $week_type; $y++) : ?>
            ...
            ?>
            <?= $date_template_engine->render('calendar/single/_day_cell', array('controller' => $controller, 'calendar' => $calendars[$y], 'em' => $tab_arr[$y], 'row' => $i, 'start' => $start * 3600, 'i' => $i + ($start * 3600 / $settings['step_week']), 'step' => $settings['step_week'], 'class_cell' => $class_cell)); ?>
        <? endfor ?>
        <? if ($rowspan > 1) : ?>
            <? if ($minutes == 0) : ?>
            <td class="precol2w"<?= $height ?>>00</td>
    ...
    <tr>
        ...
        <? if ($end < 23) : ?>
            <a href="<?= $controller->url_for('urlaubskalender/week', array('atime' => mktime($end + 1, 0, 0, date('n', $calendars[0]->getStart()), date('j', $calendars[0]->getStart()), date('Y', $calendars[0]->getStart())))) ?>">
