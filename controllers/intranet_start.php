...
class IntranetStartController extends StudipController {
    ...
    {   
        ...
        $this->intranet_buttons = IntranetIndividualButton::findBySQL('Institut_Id = ? AND content_side != ? ORDER BY `position` ASC', [$inst_id, 'right']);
        $this->intranet_right_buttons = IntranetIndividualButton::findBySQL('Institut_Id = ? AND content_side = ? ORDER BY `position` ASC', [$inst_id, 'right']);
...
//        $this->internnewstemplate->icons = $icons;
        ...
        $this->birthday_dates = [];
        //$days = [$this->today->format('d')]; 
        $days = [+$this->today->format('d')-2, +$this->today->format('d')-1, $this->today->format('d')];
        $this->today_dates = UrlaubskalenderController::getEventsByDaysAndMonth($this->calendar_sem_id, $days, $this->today->format('m'));
