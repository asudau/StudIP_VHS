...
class SeminarController extends StudipController {
    ...
    public function __construct($dispatcher)
    {
        ...
        $this->datafiel_id =  Datafield::findOneBySQL('name = \'Design f�r �bersichtsseite\'');
    ...
    public function before_filter(&$action, &$args)
    {
		...
		//falls innerhalb eines Kurses, Kursnavigation gem�� Konfiguration anpassen
		if ($this->course) 
		{
    ...
    public function index_action()
    {
        ...
        //defaultwert wenn noch nichts gew�hlt wurde
        if(!$this->style){
            $this->style = 'full';
        }
    ...
    private function get_tabs(){
        //Tabs und zugeh�rige Einstellung laden
		$position = 100;
		foreach( Navigation::getItem('course') as $key=>$tab){
		    ...
		    //Spezialfall Reiter die nur TN sehen (zB Courseware Fortschritts�bersicht)
			...
			if (!$orig_title2[0]){
				$values2 = array('id' => md5('mooc_progress'), 'tab' => 'mooc_progress', 'title' => 'Fortschritts�bersicht');
				$query2 = "INSERT INTO `system_tabs` (`id`, `tab`, `title`) VALUES (:id, :tab, :title)" ;
				$statement2 = DBManager::get()->prepare($query2);
				$statement2->execute($values2);
				$orig_title2[0]['title'] = "Fortschritts�bersicht";
		        }
